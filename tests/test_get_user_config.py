...
def test_default_config_path(user_config_path):
    ...
    assert config.USER_CONFIG_PATH == str(user_config_path)
...
def test_expand_user_for_directories_in_config(mocker):
    """Validate user pointers expanded in user configs."""

    def _expanduser(self):
        return str(self).replace('~', str(Path('/Users/bob')))

    mocker.patch.object(Path, 'expanduser', new=_expanduser)

    config_file = 'tests/test-config/config-expand-user.yaml'

    user_config = config.get_user_config(config_file)
    assert user_config['replay_dir'] == str(Path('/Users/bob/replay-files'))
    assert user_config['cookiecutters_dir'] == str(Path('/Users/bob/templates'))
...
def test_expand_vars_for_directories_in_config(monkeypatch):
    ...
    user_config = config.get_user_config(config_file)
    assert user_config['replay_dir'] == str(Path('Users/bob/cookies/replay-files'))
    assert user_config['cookiecutters_dir'] == str(Path('Users/bob/cookies/templates'))
