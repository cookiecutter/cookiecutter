...
def restore_backup_dir(original_dir, backup_dir, original_dir_found):
    ...
    # conditions.
    original_dir_is_dir = Path(original_dir).is_dir()
    backup_dir_is_dir = Path(backup_dir).is_dir()
    if original_dir_found:
        # Delete the created original_dir as long as a backup
        # exists
        if original_dir_is_dir and backup_dir_is_dir:
    ...
    if backup_dir_is_dir:
        shutil.copytree(backup_dir, original_dir)
    if Path(original_dir).is_dir():
        utils.rmtree(backup_dir)
...
def clean_system(request):
    ...
    * Restore the `~/.cookiecutter_replay/` dir from
    ...
    if user_config_path.exists():
        ...
        shutil.copy(str(user_config_path), str(user_config_path_backup))
        os.remove(str(user_config_path))
    else:
    ...
    cookiecutters_dir_backup = str(Path('~/.cookiecutters.backup').expanduser())
    ...
    # temp location
    cookiecutter_replay_dir = str(Path('~/.cookiecutter_replay').expanduser())
    cookiecutter_replay_dir_backup = str(
        Path('~/.cookiecutter_replay.backup').expanduser()
    )
    ...
    def restore_backup():
        ...
        if user_config_found and user_config_path_backup.exists():
            shutil.copy(str(user_config_path_backup), str(user_config_path))
            os.remove(str(user_config_path_backup))
