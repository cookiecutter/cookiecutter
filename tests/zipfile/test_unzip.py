...
def test_unzip_local_file(mocker, clone_dir):
    ...
    mock_prompt_and_delete = mocker.patch(
        'cookieninja.zipfile.prompt_and_delete', return_value=True, autospec=True
    )
...
def test_unzip_protected_local_file_user_password(mocker, clone_dir):
    ...
    mocker.patch('cookieninja.zipfile.read_repo_password', return_value='sekrit')
...
def test_unzip_protected_local_file_user_bad_password(mocker, clone_dir):
    ...
    mocker.patch(
        'cookieninja.zipfile.read_repo_password', return_value='not-the-right-password'
    )
...
def test_unzip_url(mocker, clone_dir):
    ...
    mocker.patch(
        'cookieninja.zipfile.requests.get',
...
def test_unzip_should_abort_if_no_redownload(mocker, clone_dir):
    ...
    mocker.patch(
        'cookieninja.zipfile.prompt_and_delete', side_effect=SystemExit, autospec=True
    )
...
def test_unzip_is_ok_to_reuse(mocker, clone_dir):
    ...
    mock_prompt_and_delete = mocker.patch(
        'cookieninja.zipfile.prompt_and_delete', return_value=False, autospec=True
    )
