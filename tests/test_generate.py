...
class TestGenerateContext(CookiecutterCleanSystemTestCase):
    ...
    def test_generate_context_with_default_and_user_parameters(self):
        context = generate.generate_context(
            context_file='tests/test-generate-context/test.json',
            default_context={"1": 3},
            user_parameters={"1": 5, "some_key": "some_other_val"}
        )
    ...
    def test_generate_context_with_user_parameters_not_in_project(self):
        with self.assertRaises(exceptions.InvalidConfiguration):
            context = generate.generate_context(
                context_file='tests/test-generate-context/test.json',
                default_context={"1": 3},
                user_parameters={"2": 5, "some_key": "some_other_val"}
            )
