...
* Cloned [cookiecutter templates](https://github.com/topics/cookiecutter-template)
  are cached into ``$XDG_CACHE_HOME/cookiecutters/`` by default, or
  ``cookiecutters_dir`` if specified. The implicit value of ``XDG_CACHE_HOME``
  is assumed to be `~/.cache` as mentioned by its
